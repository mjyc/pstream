// copied from https://github.com/probmods/webppl/blob/959c9ae0e3fb638b58186f46b057750fad986544/src/header.wppl
// added module.exports at the end
"no caching";

// Other distribution helpers

var flip = function(p) {
  var params = { p: p !== undefined ? p : 0.5 };
...
var map = function(fn, l) {
  return map_helper(0, l.length - 1, function(i) {
    return fn(l[i]);
  });
};
...
var map2 = function(fn, l1, l2) {
  return map_helper(0, l1.length - 1, function(i) {
    return fn(l1[i], l2[i]);
  });
};
...
var mapN = function(fn, n) {
  return map_helper(0, n - 1, function(i) {
    return fn(i);
  });
};
...
var SampleGuide = function(wpplFn, options) {
  return ForwardSample(
    wpplFn,
    _.assign({ guide: true }, _.omit(options, "guide"))
  );
};
...
var DefaultInfer = function(wpplFn, options) {
  ...
  var samples = 1000;
  var enumResult = Enumerate(wpplFn, {
  ...
  var rejResult = Rejection(wpplFn, {
...
var kde = function(marginal, width) {
  if (
    !(dists.isDist(marginal) && marginal.meta.name === "SampleBasedMarginal")
  ) {
