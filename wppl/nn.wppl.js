// copied from https://github.com/probmods/webppl/blob/959c9ae0e3fb638b58186f46b057750fad986544/src/nn.wppl
"no caching";
...
var gru = function(name, maybeArgs) {
  ...
  var update = compose(
    sigmoid,
    ctor(name + "update", nargs)
  );
  var reset = compose(
    sigmoid,
    ctor(name + "reset", nargs)
  );
  var candidate = compose(
    tanh,
    ctor(name + "candidate", nargs)
  );
  return function(hprev, x) {
    if (dims(hprev)[0] !== hdim) {
      error("Previous hidden vector has unexpected dimension");
...
var lstm = function(name, maybeArgs) {
  ...
  var input = compose(
    sigmoid,
    affine(name + "input", nargs)
  );
  var output = compose(
    sigmoid,
    affine(name + "output", nargs)
  );
  var candidate = compose(
    tanh,
    affine(name + "candidate", nargs)
  );
